...
function extractSlugFromHost(host, rootDomain) {
  ...
  const subPart = hostname.slice(0, hostname.length - rootDomain.length).replace(/\.$/, '');
...
function tenantResolver(options = {}) {
  const rootDomain = options.rootDomain || process.env.ROOT_DOMAIN || 'yourapp.com';
  const allowHeaderOverride =
    options.allowHeaderOverride ?? process.env.NODE_ENV !== 'production';
  ...
  return async function (req, res, next) {
    try {
      ...
      if (req.log) {
        req.log = req.log.child({ tenantId: tenant._id.toString(), tenantSlug: tenant.slug });
      }
